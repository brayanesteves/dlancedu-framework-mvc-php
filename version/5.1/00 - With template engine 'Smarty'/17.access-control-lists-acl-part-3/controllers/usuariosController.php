        ...
        public function permisos($usuarioID) {
            ...
            if($this->getInt('guardar') == 1) {
                ...
                for($i = 0; $i < count($values); $i++) {
                    if(substr($values[$i], 0, 5) == 'perm_') {
                        ...
                        if($_POST[$values[$i]] == 'x') {
                            $eliminar[] = array(
                                'usuario'    => $id,
                                // 'permiso' => substr($values[$i], -1), // Original line
                ...
                for($i = 0; $i < count($eliminar); $i++) {
                    $this->_usuarios->eliminarPermiso($eliminar[$i]['usuario'], $eliminar[$i]['permiso']);
                }

                for($i = 0; $i < count($replace); $i++) {
                    $this->_usuarios->editarPermiso($replace[$i]['usuario'], $replace[$i]['permiso'], $replace[$i]['valor']);
                }
            }
