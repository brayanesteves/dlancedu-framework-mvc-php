<?php
    class indexModel extends Model {
        ...
        public function getUsuarios() {
            $usuarios = $this->_db->query("SELECT `u`.*, `r`.`role` FROM `usuarios` `u`, `role` `r` WHERE `u`.`role` = `r`.`id_role`;");
            return $usuarios->fetchAll(PDO::FETCH_ASSOC);
        }

        public function getUsuario($usuarioID) {
            $usuarios = $this->_db->query("SELECT `u`.`usuario`, `r`.`role` FROM `usuarios` `u`, `role` `r` WHERE `u`.`role` = `r`.`id_role` AND `u`.`id` = $usuarioID;");
            return $usuarios->fetchAll(PDO::FETCH_ASSOC);
        }
